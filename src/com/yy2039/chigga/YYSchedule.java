...
public class YYSchedule
{
    ...
    public YYSchedule() {
        handler = new Handler(){
            public void handleMessage( Message msg ) {
                Log.v( "Fly", "handleMessage msg.what : " + msg.what );
                ...
                if( schedule_action != null ) {
                    ...
                    Log.v( "Fly", "handleMessage doSomething 00000000000000000000000000000000" );
    ...
    public int scheduleOnceTime( long delay, onScheduleAction schedule_action ) {
        ...
        Log.v( "Fly", "scheduleOnceTime schedule_index : " + schedule_index );
    ...
    public int scheduleCircle( long time, onScheduleAction schedule_action ) {
        final int schedule_index = nNextScheduleIndex++;
        all_schedule_actions.put( schedule_index, schedule_action );
        Log.v( "Fly", "scheduleCircle schedule_index : " + schedule_index );
    ...
    public void cancelSchedule( int schedule_index ) {
        Log.v( "Fly", "cancelSchedule schedule_index : " + schedule_index );
        onScheduleAction schedule_action = all_schedule_actions.get( schedule_index );
        if( schedule_action != null ) {
            all_schedule_actions.remove( schedule_index );
        }

        Timer timer_1 = all_schedule_timers.get( schedule_index );
        if( timer_1 != null ) {
            timer_1.cancel();
            all_schedule_timers.remove( schedule_index );
        }

        Timer timer_2 = all_schedule_timers_circle.get( schedule_index );
        if( timer_2 != null ) {
            timer_2.cancel();
    ...
    public void cancelAllSchedule() {
        Log.v( "Fly", "cancelAllSchedule 3333333333333333333333333333333333333333333333333333333" );
